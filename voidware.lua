...
local cfg = {
    ...
    -- aim
    silent = false, silentFov = 300, silentPart = "Head",
    wallbang = false,
    ...
    if cfg.orbit and cfg.selected then
        ...
        if tp and tp.Character then
            ...
            if tHRP then
                ...
                if cfg.oFace then r.CFrame = CFrame.lookAt(np, tPos)
                else local tan=Vector3.new(-math.sin(cfg.oAng),0,math.cos(cfg.oAng))*dir ; r.CFrame=CFrame.lookAt(np,np+tan) end
                pcall(function() r.Velocity=V3ZERO end)
            end
        end
    end
...
cL:AddSlider('s_fov',{Text='fov',Default=300,Min=50,Max=750,Rounding=0,Callback=function(v) cfg.silentFov=v end})
